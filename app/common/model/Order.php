...
class Order extends Model
{
    ...
    public function createNewOrder($uid, $extra_data,$lang)
    {
        ...
        //开启事务
        $this->startTrans();
        // 行级锁定：锁定用户行与该用户未完成订单集合，防止并发创建重复订单
        try {
            // 锁用户行，确保并发时顺序化
            Db::table('mod_user')->where('id', $uid)->lock(true)->find();
            // 在事务内再次校验是否已有未完成订单（加锁）
            $existingUnfinished = $this->where(['uid' => $uid, 'status' => 1])->lock(true)->count();
        ...
        try {
            // 插入订单（若后续为 order_no 加唯一索引，这里可天然避免重复）
    ...
    {
        // 先开启事务并对订单行加锁，避免并发重复处理
        $this->startTrans();
        try {
            // 对订单行加行级锁，确保并发时只有一个事务可以继续
            $order_info = $this->where('id', $order_id)->lock(true)->find();
        } catch (\Exception $e) {
            $this->rollback();
            throw $e;
        }
        ...
        try {
            ...
            if ($order_info['status'] != 2) {
                ...
                // 幂等：若存在唯一约束 (order_id,type,uid) 将天然避免重复；无约束时做一次存在性检查
                if (!WalletLog::where(['order_id' => $order_info['id'], 'type' => 3, 'uid' => $uid])->find()) {
                    WalletLog::create(array_merge($log, ['order_id' => $order_info['id']]));
                }
            }
