...
class Index extends Base
{
    ...
    public function login()
    {
        ...
        $langs = $this->request->post('lang', '', 'trim');
        if (!$username)
        ...
        $info = User::whereRaw("$binary username = '$username'")
            ...
            ->find();
        if (!$info) {
			if($langs == 'zh')$langs = '用户名或者密码错误';//中文
			if($langs == 'pt')$langs = 'Nome de utilizador ou palavra-passe incorretos';//葡萄牙语
			if($langs == 'nl')$langs = 'Verkeerde gebruikersnaam of wachtwoord';//荷兰语
			if($langs == 'he')$langs = 'שם משתמש או סיסמה שגויים';//希伯来语
			if($langs == 'fr')$langs = 'Nom d utilisateur ou mot de passe incorrect';//法语
			if($langs == 'es')$langs = 'Por favor, complete primero la información de retiro';//西班牙语
			if($langs == 'en')$langs = 'Wrong username or password';//英语
			if($langs == 'de')$langs = 'Falscher Benutzername oder falsches Passwort';//德语
			if($langs == 'ar')$langs = 'ياسم المستخدم أو كلمة المرور خاطئة';//阿拉伯语
        ...
        if ($info['status'] == 2) {
			if($langs == 'zh')$langs = '该账号被禁用了，请联系客服';//中文
			if($langs == 'pt')$langs = 'Esta conta foi desativada, contacte o serviço de apoio ao cliente';//葡萄牙语
			if($langs == 'nl')$langs = 'Dit account is uitgeschakeld, neem contact op met de klantenservice';//荷兰语
			if($langs == 'he')$langs = 'אחשבון זה הושבת, אנא צור קשר עם שירות הלקוחות';//希伯来语
			if($langs == 'fr')$langs = 'Ce compte a été désactivé, veuillez contacter le service client';//法语
			if($langs == 'es')$langs = 'Esta cuenta ha sido deshabilitada, por favor contacte con atención al cliente';//西班牙语
			if($langs == 'en')$langs = 'This account has been disabled, please contact customer service';//英语
			if($langs == 'de')$langs = 'Dieses Konto wurde deaktiviert. Bitte wenden Sie sich an den Kundendienst.';//德语
			if($langs == 'ar')$langs = 'يتم تعطيل هذا الحساب، يرجى الاتصال بخدمة العملاء';//阿拉伯语
            $this->result(false, $langs);
        }
